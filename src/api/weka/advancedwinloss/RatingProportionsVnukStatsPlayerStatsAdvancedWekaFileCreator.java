...
public class RatingProportionsVnukStatsPlayerStatsAdvancedWekaFileCreator extends
	...
	protected String getDataForMatch(int leagueId, int matchId, String directoryPath)
		...
		TeamDetails homeTeamDetails = hoc.getTeamDetailsFromFile(matchDetails.getMatch().getHomeTeam().getTeamID());
		TeamDetails awayTeamDetails = hoc.getTeamDetailsFromFile(matchDetails.getMatch().getAwayTeam().getTeamID());
		
		
		MatchLineup homeMatchLineup = hoc.getMatchLineupFromFile(leagueId, matchId, homeTeamDetails.getTeamId());
		MatchLineup awayMatchLineup = hoc.getMatchLineupFromFile(leagueId, matchId, awayTeamDetails.getTeamId());
		...
		for(Player player : homeMatchLineup.getTeam().getLineup())
		{
			...
			if (generalRole == MatchRoleID.MIDFIELD_ROLE)
			{
				homeMidCounter++;
			...
			if (generalRole == MatchRoleID.DEFENDER_ROLE)
			{
				homeDefCounter++;
			...
			if (generalRole == MatchRoleID.ATTACKER_ROLE)
			{
				homeAttCounter++;
